...
The following procedure will focus on setting up your brand new {partner-product-name} deployment.

include::./_config_application.adoc[]

=== Adding nodes to the {partner-product-name} cluster

By default, your Quick Start deployment will start with a single {partner-product-name-short} node (Auto Scaling group of `min=1` and `max=1`). After you're ready to add nodes to your cluster, perform the following steps:
